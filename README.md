...
```bash
uv export --frozen --extra dev --no-hashes --no-editable -o requirements-ci.txt
```
...
- `--no-editable` — converts the local project entry from `-e .` to `.` so the file is self-contained
