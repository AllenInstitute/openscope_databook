...
First try the lockfile path (`uv sync --frozen --extra dev --python 3.10`), which mirrors CI. If you need a non-uv path, use a clean Python 3.10 virtual environment and install with `python -m pip install -r requirements-ci.txt` followed by `python -m pip install -e .`. Conda is still a valid option for environment isolation if you prefer it.
